...
public class GerenciadorDeLembretes {
	...
	public void gerenciar() {
		...
		while (true) {
			System.out
					.println("Digite a operação que deseja realizar:\n 1 - Para realizar login.\n 2 - Para criar um login.");
			op = sc.nextInt();
				...
				this.autenticar();
							...
							while (true) {
								...
								if (op != 0) {
									...
									case 1:
										System.out
												.println("Este lembrete possui data de expiração?\n(Digite '1' se houver ou '2' se não houver)");
										op = sc.nextInt();
										if (op == 1) {
											System.out
											...
											System.out
													.println("Digite o data de expiração dessa tarefa (em número): ");
											...
											System.out
													.println("Digite a descrição dessa tarefa (Ou digite 'n' caso não haja descrição): ");
											String descricao = sc.next();
											System.out
													.println("Digite um local para essa tarefa (Ou digite 'n' caso não haja local): ");
											String local = sc.next();
											System.out
													.println("Digite ums tag para essa tarefa: ");
											String tag = sc.next();

											if (!descricao.equals("n")
													&& !local.equals("n")) {
												Notas tarefa = new Tarefas(
														titulo, expData, tag,
														descricao, local);
												System.out
														.println("titulo, expData, tag, descricao, local"); // Para
																											// testar
																											// os
																											// ifs
											} else if (descricao.equals("n")
													&& !local.equals("n")) {
												Notas tarefa = new Tarefas(
														titulo, expData, tag,
														local);
												System.out
														.println("titulo, expData, tag, local"); // Para
																									// testar
																									// os
																									// ifs
											} else if (!descricao.equals("n")
												...
												Notas tarefa = new Tarefas(
														...
														descricao);
												System.out
														.println("titulo, expData, tag, descricao"); // Para
																										// testar
																										// os
																										// ifs
											} else if (descricao.equals("n")
												...
												Notas tarefa = new Tarefas(
														titulo, expData, tag);
												System.out
														.println("titulo, expData, tag"); // Para
																							// testar
																							// os
																							// ifs
											}
											...
											if (!descricao.equals("n")
													&& !local.equals("n")) {
												Notas lembrete = new Lembretes(
														titulo, tag, descricao,
														local);
												System.out
														.println("titulo, tag, descricao, local"); // Para
																									// testar
																									// os
																									// ifs
											} else if (descricao.equals("n")
												...
												Notas lembrete = new Lembretes(
														titulo, tag, descricao);
												System.out
														.println("titulo, tag, descricao"); // Para
																							// testar
																							// os
																							// ifs
											} else if (descricao.equals("n")
									...
									case 4:
										System.out
												.println("Tem certeza que deseja deletar o usuário '"
														+ user.getLogin(usuario)
										...
										if (op == 2) {
											System.out
													.println("O usuário não foi deletado.\n---------------------------");
											break;
										}
...
//				}
			...
			case 2:
				if (numLog < 51) {
					...
					System.out
							.println("Não há mais espaço para usuários nesse aplicativo. Delete algum usuário.");
				}
				break;
			...
			while(true){
				try {
					...
					if (entradaL.equals('n'))
						return null; // se retornar nullo, tem que voltar para o menu inicial.
