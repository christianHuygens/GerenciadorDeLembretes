		...
		while(true){
			...
			case 1:
				if (numLog > 0){
					...
					entradaL = sc.next();
					usuario = login.digitarUsuarioLogin(entradaL, numLog);
					if (login.getLogin(usuario).equals(entradaL)){
						System.out.println("Digite sua senha:");
						entradaS = sc.next();
						senhaUsuario = login.digitarSenhaLogin(usuario, entradaS);
						if (senhaUsuario == true){
							...
							while(true){
								System.out.println("Olá "+login.getLogin(usuario)+".\nEsses são todos os seus lembretes.");
								...
								op = sc.nextInt();
									...
									//////// MENU DE OPERAÇÕES
										...
										case 4:
											System.out.println("Tem certeza que deseja deletar o usuário '"+login.getLogin(usuario)+"'?\nDigite '1' para confirmar ou '2' para desistir)");
											op = sc.nextInt();
											if(op == 1){
			...
			case 2:
				if (numLog < 51){
					...
					entradaL = sc.next();
					login.novoLogin(numLog, entradaL);
					System.out.println("Digite a sua senha:");
					entradaS = sc.next();
					login.novoLoginSenha(numLog, entradaS);
					System.out.println("Usuário '"+ login.getLogin(usuario) +"' cadastrado com sucesso.\n---------------------------");
